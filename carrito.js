...
payMethod.addEventListener('change', ()=>{
  ...
  if(metodo === 'Yape' || metodo === 'Plin'){
    ...
    // AHORA PEDIMOS EL CÓDIGO DE OPERACIÓN PARA VALIDAR
    paymentExtra.innerHTML = `
    ...
    // AÑADIMOS IDs A LOS INPUTS PARA PODER LEERLOS LUEGO
    paymentExtra.innerHTML = `
...
btnPay.addEventListener('click', ()=>{
  ...
  // 1. Validar que haya un método seleccionado
  if(!metodo) return alert('⚠️ Selecciona un método de pago');

  // 2. VALIDACIÓN PARA TARJETA
  if (metodo === 'Tarjeta') {
  ...
  // SI PASA LAS VALIDACIONES, PROCESAMOS EL PAGO
